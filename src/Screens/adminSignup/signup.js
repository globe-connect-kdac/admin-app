...
const Signup = () => {
  ...
  const onSignUpClicked = async () => {
    // Regex for normal password: at least 6 characters, 1 lowercase letter, and 1 uppercase letter
    const normalPasswordRegex = /^(?=.*[a-z])(?=.*[A-Z]).{6,}$/;
    ...
    if (userName.length === 0) {
      ...
      toast.warn('Password must be at least 6 characters long and include one uppercase and one lowercase letter');
